...
class LeagueAPI extends RateLimiter {
  ...
  LeagueAPI({
    ...
    matchServer = serverMap[server];

    //custom rate limit stuff
    appMaxCallsPerSecond = appLowerLimitCount;
    appMaxCallsPerTwoMinutes = appUpperLimitCount;

    getIt.registerSingleton<UrlHelper>(urlHelper);
    getIt.registerSingleton<DDragonStorage>(dDragonStorage);
    urlHelper.apiKey = apiToken;
  }
  ...
  Future<LeagueResponse> getMatchesFromAPI(String puuid, {int start = 0, int count = 100}) async {
    var url =
  ...
  Future<LeagueResponse> getCurrentGame(Summoner summoner) async {
    ...
    final response = await makeApiCall(url, APIType.match);
  ...
  Future<LeagueResponse> makeApiCall(String url, APIType apiType) async {
    ...
    if (!passesApiCallsRateLimit(now)) returnLeagueResponse(responseCode: 429, retryTimestamp: now + 30000);
    if (!passesHeaderRateLimit(now)) returnLeagueResponse(responseCode: 429, retryTimestamp: now + 30000);
    ...
    if (response.statusCode == 200) {
      ...
      return returnLeagueResponse(responseCode: response.statusCode, retryTimestamp: retryTimeStamp);
    } else {
  ...
  /// ```
  /// method to get their champion name, level and if chest aquired.
  Future<List<ChampionMastery>?> getChampionMasteriesFromAPI({String? summonerID}) async {
    var url =
  ...
  Future<List<LeagueEntryDto?>?> getRankedQueueFromAPI(String queue, String tier, String division,
      {int page = 1}) async {
    var url =
        'https://$server.api.riotgames.com/lol/league-exp/v4/entries/$queue/$tier/$division?page=$page&api_key=$apiToken';
...
enum Queue { RANKED_SOLO_5X5, RANKED_FLEX_SR }
